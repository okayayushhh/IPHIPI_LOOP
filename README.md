...
🎥 [Demo video](https://youtu.be/IasfQEbF0b4) · 📐 [Architecture](docs/ARCHITECTURE.pdf) · 📊 [Sample I/O](docs/sample_outputs/)
...
**LLM:** Gemini 2.5 Flash (Lite for hot path, Pro for synthesis)
...
Then open `.env` in a text editor and paste your real keys. You need a
**Gemini API key** (free tier: 1000 requests/day). See `.env.example` for
sign-up links.
...
| `docs/` | Architecture document + sample outputs |
...
| `app/services/gemini_client.py` | LLM client wrapper |
...
| Path | Purpose |
|---|---|
| `ARCHITECTURE.pdf` | Five-page architecture document |
...
the same résumé, you'll get *similar* outputs, but exact scores and phrasings
will vary by run.
